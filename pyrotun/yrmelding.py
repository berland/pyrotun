...
def map_weatherdict_to_openhab(weather):
    return {"YrmeldingNaa": weather["weatherquality"][0],
...
def get_weather(xmlfile=None):
    ...
    nexthours = [int(root.find('forecast').find('tabular').findall('time')[x].symbol.attrib['number']) for x in range(0, 6)]
    nexttemps = [float(root.find('forecast').find('tabular').findall('time')[x].temperature.attrib['value']) for x in range(0,6)]


    currenttext = root.find('forecast').find('tabular').findall('time')[0].symbol.attrib['name']
    nexthourtext = root.find('forecast').find('tabular').findall('time')[1].symbol.attrib['name']

    return {"weatherquality": nexthours,
